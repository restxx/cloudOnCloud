 ...
 * @version $Id$
 */
class SCloudDev extends FirmAbstract
{
    use Financail;
    ...
    //test
    //protected $host = 'http://192.168.97.74:8181';
    protected $regions = [];
    protected $projects = [];
    ...
    public function __construct($params)
    {
        ...
        $writer = new Stream(APP.DIRECTORY_SEPARATOR.'runtime'.DIRECTORY_SEPARATOR.'logs'.DIRECTORY_SEPARATOR.'SCloudDev.log');
        ...
        $opts      = array(
            'http' => array(
                ...
                'header'  => "Content-type: application/json\r\n" .
                    "Sp-Agent: openstack_kilo\r\n" .
                    "X-Auth-Token: " . $this->getToken(),
                'content' => $data,
            )
        );
        $this->logger->log(Logger::INFO, '[POST] '.$url);
        $this->logger->log(Logger::INFO, '[request_body] '.json_encode($opts));
        $cxContext = stream_context_create($opts);
        $result    = @file_get_contents($url, false, $cxContext);
        $this->logger->log(Logger::INFO, '[response] '.$result);
    ...
    public function get($url)
    {
        $opts = array(
            'http' => array(
                'timeout' => 30,
                'header'  => "Sp-Agent: openstack_kilo\r\nX-Auth-Token: " . $this->getToken(),
            )
        );
        $this->logger->log(Logger::INFO, '[GET] '.$url);
        $this->logger->log(Logger::INFO, '[request_body] '.json_encode($opts));
        $cxContext = stream_context_create($opts);
        $result    = @file_get_contents($url, false, $cxContext);
        $this->logger->log(Logger::INFO, '[response] '.$result);
    ...
    {
        $sessionKey = "firm_scloud_dev_token_" . $this->username;
